...
//            print_vector(distancias);
    ...
    //Ver si la tupla ya esta en la lista de Reomendados
    //Si ya esta, calcular un nuevo puntaje
    //Si no, agregar la nueva tupla
    void IngresarItem(vector <tuple<float,string,int>> &Recomendados, tuple<float,string> Nuevo)
    {
      if(Recomendados.size()==0){
        //cout<<"Deberia entrar solo una vez"<<endl;
        Recomendados.push_back(make_tuple(get<0>(Nuevo),get<1>(Nuevo),1));
      }
      else
      {
        for ( int i=0; i<Recomendados.size(); i++)
        {
          //cout<<get<1>(Recomendados[i])<<" , "<<get<1>(Nuevo);
          if(get<1>(Recomendados[i]) == get<1>(Nuevo))
          {
            get<2>(Recomendados[i]) = get<2>(Recomendados[i]) + 1;
            //cout<<" contador: "<< get<2>(Recomendados[i]);
            get<0>(Recomendados[i]) = (get<0>(Recomendados[i])*(get<2>(Recomendados[i])-1) + get<0>(Nuevo))/get<2>(Recomendados[i]);
            break;
          }
          else if(i == Recomendados.size()-1)
          {
            //cout<<"Primera vez que recomienda: "<<get<1>(Nuevo)<<endl;
            Recomendados.push_back(make_tuple(get<0>(Nuevo),get<1>(Nuevo),1));
            break;
          }
        }
    ...
    vector <tuple<float,string,int>> RecomendarPorKUsuarios(string usuario, string algoritmo, int k,float umbral)
    {
      ...
      for (int i=0; i<MasCercanos.size();i++)
      {
        usuario2 = get<1>(MasCercanos[i]);
        //cout<<"\nUsuario más cercano: "<<usuario2<<endl;
        ...
        for (int j=0; j<ItemsMayorMenor.size();j++)
        {
          if(get<0>(ItemsMayorMenor[j])>=umbral)
          {
            ...
            try{
              ...
              // puntaje = data[usuario2].at(get<1>(ItemsMayorMenor[j]));
              // Resultado.append(make_tuple(puntaje,get<1>(ItemsMayorMenor[j])));
              //cout<<"\n  No vio la pelicula: "<<get<1>(ItemsMayorMenor[j])<<", "<<usuario2<<" la puntua con: "<<get<0>(ItemsMayorMenor[j])<<endl;
              IngresarItem(Recomendados,ItemsMayorMenor[j]);
            }
          }
        }
